...
function loadData(dataGenerator){
	var counter = 0;
	var aDistinctCreditorAccountNum = genObj.getTableColumnAsDistinctArray('"CIO"."CIO.GenerateData.DB.Tables::CreditorAccount"','CreditorAccNumber',' 1=1 ');

	for(var i = 0 ; i < genObj.noOfRecords; i++){

		var sql = 	'insert into "CIO"."CIO.GenerateData.DB.Tables::PurchaseOrder" values(?,?,?,?,?,?,?,?,?)';
