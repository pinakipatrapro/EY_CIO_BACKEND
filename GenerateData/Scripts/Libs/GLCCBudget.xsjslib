...
function loadData(dataGenerator){
	...
	var aBudgetType = ['Project','Operation'];
		...
		for(var j = 0 ; j < aDistinctCC.length; j++){
			for(var k = 0 ; k < aYear.length; k++){
				for(var l = 0 ; l < aMonth.length; l++){
					for(var m = 0;m<aBudgetType.length;m++){
						...
						statement.setString(4,aBudgetType[m]);
						statement.setDecimal(5,amount);
						statement.setString(6,curr);
