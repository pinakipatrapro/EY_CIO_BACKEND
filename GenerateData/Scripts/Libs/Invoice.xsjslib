function loadData(dataGenerator){
	...
	var tableName = '"CIO"."CIO.GenerateData.DB.Tables::PurchaseOrder"';
	var aCreditorAccountNum = genObj.getTableColumnAsArray(tableName,'CreditorAccountNumber',' 1=1 ');
	var poDocNumber = genObj.getTableColumnAsArray(tableName,'PODocumentNumber',' 1=1 ');
	...
	for(var i = 0 ; i < poDocNumber.length; i++){
		...
		var sql = 	'insert into "CIO"."CIO.GenerateData.DB.Tables::InvoiceDocument" values(?,?,?,?,?,?,?,?,?,?,?)';
